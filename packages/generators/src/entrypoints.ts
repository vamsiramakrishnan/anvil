...
export function generateRuntimeServer(air: AirDocument): string {
  ...
  const webhookRoutes = resolvedWebhookRoutes(air);
  return `#!/usr/bin/env node
...
import {
  ...
  recordWebhookCompletionIfIndexed,
  verifyInboundToken,
...
// AsyncContract resolved a real webhook. The FULL WebhookContract is not
// embedded here; it is re-derived at request time from the loaded 'air' by
// resolveAsyncContract, the exact same pure function every other surface
// (the MCP tool metadata, certification) resolves it through, so a stale
// embedded copy can never diverge from what the rest of the runtime agrees
// the contract is.
const webhookRoutes = ${JSON.stringify(webhookRoutes)};
...
async function resolveWebhookRef(ref) {
...
function webhookRequestUrl(req) {
...
function readRawBody(req, res) {
  ...
  if (
    ...
    req.resume();
    json(res, 413, { error: { code: "validation_error", message: "Webhook body exceeds the size limit." } });
...
async function handleWebhookRoute(req, res, route) {
  const submitOp = air.operations.find((op) => op.id === route.submitOperationId);
  const allOpsById = new Map(air.operations.map((op) => [op.id, op]));
  const resolution = submitOp ? resolveAsyncContract(submitOp, allOpsById) : { ok: false };
  if (!resolution.ok || !resolution.contract.webhook || !resolution.webhookOperation) {
    ...
    // guess through.
    return json(res, 404, { error: { code: "not_found", message: "Webhook route is not resolvable." } });
  }
  ...
  return json(res, outcome.status, {
    error: { code: outcome.status === 401 ? "auth_required" : "validation_error", message: outcome.reason },
  });
}
