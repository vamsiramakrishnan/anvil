 ...
 */
import { type AirDocument, kebabCase } from "@anvil/air";

/**
 * The Claude Code plugin name for a service — the `.claude-plugin` identity.
 * Claude requires `name` to be a kebab-case slug, but a service id derived from a
 * title is snake_case (`snakeCase("Payments API")` → `payments_api`), so it must
 * be kebab-cased here — `anvil-payments_api` is an invalid, unloadable manifest.
 * The plugin-scoped tool matcher is built from this same value, so it stays
 * consistent.
 */
export function pluginName(air: AirDocument): string {
  return `anvil-${kebabCase(air.service.id)}`;
}
...
//
// Codex's PreToolUse contract (https://developers.openai.com/codex/hooks) supports
// only two decisions: "deny", and "allow" WITH updatedInput (a rewrite). A bare
// "allow" is unsupported — Codex marks the hook run failed and RUNS THE TOOL
// ANYWAY — and there is no interactive "ask". So this shim only ever emits "deny"
// or no decision:
//   - deny (unknown / unapproved / model-confirm-missing / idempotency) -> deny.
//     A model-confirm op is denied until the model re-invokes with confirm:true.
//   - ask (human-approval) -> ALSO deny, fail-closed. Codex cannot prompt a human
//     from PreToolUse and the model must never self-approve, so the only honest
//     enforcement is to block the tool here. Interactive human approval in Codex
//     is its own approval flow / the PermissionRequest hook, not this one.
//   - allow -> emit NO permissionDecision (just additionalContext if any). Never
//     a bare "allow", which Codex would reject; absence lets the tool proceed.
import { decide } from "../hookcore.mjs";
...
if (d.decision === "deny" || d.decision === "ask") {
  ...
  out.hookSpecificOutput.permissionDecisionReason =
    d.decision === "ask"
      ? \`\${d.reason || ""} Codex cannot prompt for human approval from a PreToolUse hook, so this operation is blocked here — run it with a human in the loop via Codex's own approval flow.\`.trim()
      : d.reason || "";
} else if (d.context) {
  // allow: never emit a bare permissionDecision "allow" (unsupported). Just add
  // context and let Codex proceed.
...
- Codex PreToolUse supports only \`deny\` and \`allow\`+\`updatedInput\` (a rewrite);
  a **bare \`allow\` is unsupported** — Codex marks the hook failed and runs the
  tool anyway — and there is no interactive \`ask\`. So this hook never emits a
  bare allow: a clean call gets **no decision** (the tool proceeds), and every
  gate is a \`deny\`.
- The two gate tiers therefore map like this in Codex:
  - **model-confirm** — denied pre-flight until the model re-invokes with
    \`confirm: true\` (then it proceeds).
  - **human-approval** (\`--human-approval\` / manifest \`human_approval\`) — denied
    **fail-closed**. Codex cannot prompt a human from a PreToolUse hook and the
    model must never self-approve, so the only honest enforcement is to block the
    tool. These operations are therefore **not runnable autonomously in Codex** —
    to human-gate them interactively, use Codex's own approval policy / the
    \`PermissionRequest\` hook, which is the mechanism designed for user approval.
