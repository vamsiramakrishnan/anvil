...
//
// Codex's PreToolUse contract documents "deny"/"allow" (no "ask"). hookcore
// returns "ask" ONLY for human-approval operations, and returns it even when
// confirm:true is supplied (the model cannot self-confirm past it). A "deny"
// there would be TERMINAL — no flag clears it, so re-invoking repeats the denial
// and the tool is bricked. Those map to "allow" instead, surfacing the
// human-approval requirement as context and deferring to Codex's own permission
// flow (PermissionRequest) as the human gate. Model-confirm gates stay "deny":
// the model re-invokes with confirm:true, which hookcore then allows. The output
// shape mirrors Claude's; if the installed Codex build differs, adjust it against
// the current Codex hooks reference (see README.md).
import { decide } from "../hookcore.mjs";
...
const d = decide(event.tool_name, event.tool_input || {});
// "ask" == human-approval (see above). Never a terminal deny — allow and let
// Codex's own approval flow gate it.
const humanApproval = d.decision === "ask";
const decision = humanApproval ? "allow" : d.decision;
const reason = humanApproval
  ? \`\${d.reason || ""} Codex must obtain explicit human approval before running this — do not auto-approve.\`.trim()
  : d.reason || "";
...
// Carry the human-approval requirement forward even on allow, so the reason is
// visible to Codex's permission flow and the user.
const context = humanApproval ? d.reason || d.context : d.context;
...
- Codex documents \`deny\`/\`allow\` (not \`ask\`) for PreToolUse, so the two gate
  tiers map differently: a **model-confirm** mutation is denied pre-flight until
  the model re-invokes with \`confirm: true\`; a **human-approval** operation
  (\`--human-approval\` / manifest \`human_approval\`) is *allowed* with the
  approval requirement in \`permissionDecisionReason\`/\`additionalContext\`, and
  Codex's own \`PermissionRequest\` flow is the human gate. Human-approval ops are
  deliberately NOT denied — a deny would be terminal (no flag clears it) and would
  brick the tool rather than gate it.
