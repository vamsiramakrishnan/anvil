...
function cmdAssess(args: string[], flags: Record<string, string | boolean>, io: CliIO): number {
  ...
  if (!path) {
    io.err(
      "Usage: anvil assess <dir|air.yaml> [<operation>] [--severity S] [--check [--fail-on D]] [--json]",
    );
    return 1;
  }
  const checking = flags.check === true;
  if (flags["fail-on"] !== undefined && !checking) {
    io.err("--fail-on only applies with --check (a report never gates).");
    return 1;
  }
  const failOn = typeof flags["fail-on"] === "string" ? (flags["fail-on"] as string) : "blocked";
  const failing = FAIL_ON[failOn];
  if (!failing) {
    io.err(`Unknown --fail-on '${failOn}'. Use one of: ${Object.keys(FAIL_ON).join(", ")}.`);
    return 1;
  }
  const air = loadAir(path);
  ...
  if (filtered && !(SEVERITIES as readonly string[]).includes(flags.severity as string)) {
    io.err(`Unknown severity '${flags.severity}'. Use one of: ${SEVERITIES.join(", ")}.`);
    return 1;
  }
  const view = viewAssessment(
    assessment,
    filtered ? { minimumSeverity: flags.severity as Severity } : {},
  );
