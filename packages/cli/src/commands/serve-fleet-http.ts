...
export async function startFleetHttp(
  ...
  const server: Server = createServer(async (req, res) => {
    ...
    if (!session) {
      ...
      // The session's principal: its bearer token looked up in the directory.
      // Unconfigured directory → undefined → execute()'s anonymous default;
      // configured directory + unlisted token → undefined + directoryConfigured
      // → execute() refuses fail-closed (policy/principal_unresolved).
      const principal = principalDirectoryConfigured
