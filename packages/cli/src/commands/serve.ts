...
import type { Principal, RuntimeConfig } from "@anvil/runtime";
...
export async function prepareFleetForWorkspace(
  ...
  const { allowedHostsFor, bootRuntimeFromEnv, buildLimitsGate } = await import("@anvil/runtime");
  ...
  // names any entries at all, so an unresolved principal (a mistyped/missing
  // credential, or one the directory doesn't name) is refused fail-closed
  // instead of silently reproducing the anonymous default (see `execute()`'s
  // principal-resolution gate, `@anvil/runtime`).
