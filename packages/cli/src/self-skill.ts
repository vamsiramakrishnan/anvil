...
| \`oauth2_authorization_code\` | \`ANVIL_<PROFILE>_TOKEN\` (replay), or \`ANVIL_<PROFILE>_REFRESH_TOKEN\` + \`_CLIENT_ID\` (+ optional \`_CLIENT_SECRET\`) | The runtime never runs the interactive step. Run \`anvil auth login <bundle> --profile <profile>\` once — it completes the PKCE authorization-code flow (RFC 6749 §4.1, RFC 7636) on a loopback broker and writes \`{ refresh_token, obtained_at }\` to \`~/.anvil/credentials/<profile>.json\` (mode 0600), which the runtime reads when \`_REFRESH_TOKEN\` is unset. \`anvil auth status <bundle>\` lists profiles and whether material is present (names only). End-user authority is a human decision: a bare manifest \`state: approved\` never moves this type past \`review_required\`, however complete its material is. A manifest MAY grant it by naming who decided and why on the same entry — \`reviewed_by\` and \`review_reason\` both non-empty (see \`docs/MANIFEST.md\`) — the only approval path on a receipt-bound gateway bundle, where \`anvil approve\` refuses in place. |
